			...
			if(pd_token!=null){
				if(cmd.equals("DocAll")){  //根据提交的方法执行相应的操作，获取知识库分页信息 
					...
					if(json.getString("validate")!=null&& !json.getString("validate").equals("")){
						//pd.put("validate",Tools.getEndTime(json.getString("validate"), 1));
						pd.put("validate",json.getString("validate"));
					}
					if( json.getString("doctype")!=null&& !json.getString("doctype").equals("")){
						pd.put("doctype", json.getString("doctype"));
					}
