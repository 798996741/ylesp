		...
		try{
			...
			if(pd_token!=null){
				if(cmd.equals("employregAll")){  //根据提交的方法执行相应的操作，获取知识库分页信息 
		            ...
		            for(PageData xp:list){
		            	if(xp.get("agestart")!=null&&xp.get("agestart").equals("0")&&xp.get("ageend")!=null&&xp.get("ageend").equals("100")){
				        	xp.put("age","不限");
				        }else if(xp.get("agestart")!=null&&xp.get("ageend")!=null){
				        	xp.put("age",xp.get("agestart")+"-"+xp.get("ageend"));
				        }
				        
		            	if(xp.getString("age")!=null&&xp.getString("age").equals("0-100")){
		            		xp.put("age","不限");
		            	}
		            }
