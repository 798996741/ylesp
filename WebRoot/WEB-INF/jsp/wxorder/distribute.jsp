...
<script type="text/javascript">
	...
	function handle(pid){
		...
		var userid='${pd.userid}';
		var id=$("#id").val();
		var data="{\"api\":\"com.yulun.WorkorderWeb\",\"data\":{\"username\":\""+userid+"\",\"msg\":\"yes\",\"OPINION\":\"tongyi\",\"PROC_INST_ID_\":\""+pid+"\",\"id\":\""+id+"\"},\"cmd\":\"WorkorderSh\"}";
		//alert(data);
		//return false;
		$.ajax({
        	//几个参数需要注意一下
            type: "POST",//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: "<%=path%>/appuser/workorderSh.do" ,//url
            data:{"data":data}, //参数值
            success: function (result) {
	...
	function sh(doaction){
		...
		msgalsert(tsdate,"请选择投诉时间");
		var tsman=$("#tsman").val();
		var tstel=$("#tstel").val();
		var tscont=$("#tscont").val();
		msgalsert(tscont,"请输入投诉内容");
		var tslevel=$("#tslevel").val();
		msgalsert(tslevel,"请选择投诉等级");
		var tsdept=getCheckBoxValueThree();
		msgalsert(tsdept,"请选择投诉部门");
		var tstype=$("#tstype").val();
		msgalsert(tstype,"请选择投诉分类");
    ...
    <div class="detail-bg">
        ...
        <section>
            ...
            <div class="bottom-btn">
            	<div class="bottom-deal" onclick="kscl()" id="kscl">
             		快速处理
                </div>
                <div class="bottom-deal" id="jsgd" onclick="sh('0')" style="display:none">
             		结束工单
                </div>
                <div class="bottom-cfts"  onclick="complain()">
                                                   重复投诉
                </div>
                ...
                <div class="bottom-bc" style="right: 5%;">
