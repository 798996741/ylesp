		...
		try{
			...
			if(varList.size()>0){
				...
				for(PageData pddata:varList){
					...
					tbodystr+="<td class='center cy_td'>"+(pddata.get("CODE")==null?"":pddata.get("CODE"))+"</td>";
					...
					if(action.equals("")){
						...
						//if(pddata.getString("TYPENAME").indexOf("单")>=0||pddata.getString("TYPENAME").indexOf("判断")>=0){
							tbodystr+= "<a  class=\"cy-pzd\" onclick=\"setnext('"+pddata.getString("ID")+"','"+pddata.get("CODE")+"');\"> <i title=\"设置下一题\"></i></a>";			
						//}
