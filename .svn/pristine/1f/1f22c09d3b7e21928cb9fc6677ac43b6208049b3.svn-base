...
<script type="text/javascript">
            ...
            success: function (obj) {
                ...
                if(obj.success=="true"){
                    ...
                    for(var o in data){ 
                    	...
                    	str=str+"<td><div class=\"cell\"><span class=\"btn-opt bg-blue\" onclick=\"downfile("+data[o].id+")\">下载</span>&nbsp;&nbsp;<span class=\"btn-opt bg-red\" onclick=\"delfile("+data[o].id+")\">删除</span> </div></td>";
                    	str=str+"</tr>";	
                    }
                }
	...
	function back(){
		...
		if(action=='pf'){
			var id='${pd.id}';
			location.href="<%=path%>/appWeixin/distribute?id="+id+"&userid="+userid+"";
		}else{
			location.href="<%=path%>/appWeixin/complandetail_sh?ID_="+ID_+"&proc_id="+proc_id+"&userid="+userid+"";
		}
	}
...
<body onload="loadWork()">
	<div class="ada-table ada-table-bluebg">
		...
		<c:if test="${empty pd.action||pd.action=='pf'}">
			<div class="bottom-upload" onclick="attupload()">上传附件</div>
		</c:if>
		<div class="bottom-back" onclick="back();">返回上一页</div>
	</div>
</body>
