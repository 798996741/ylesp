...
public class DictionariesService implements DictionariesManager{
	...
	public PageData findByDictName(PageData pd)throws Exception{
		return (PageData)dao.findForObject("DictionariesMapper.findByDictName", pd);
	}
	/**通过编码获取数据
	 * @param pd
	 * @throws Exception
	...
	public List<PageData> listAllDictByParentId(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("DictionariesMapper.listAllDictByParentId", pd);
	}
	
	public List<PageData> listAllDictByBM(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("DictionariesMapper.listAllDictByBM", pd);
	}

	public List<PageData> listAllDictBySql(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("DictionariesMapper.listAllDictBySql", pd);
	}
	
	public List<Dictionaries> listAllDictByPlanType(String parentId, String action) throws Exception {
		List<Dictionaries> dictList = this.listSubDictByParentId(parentId);
		for(Dictionaries dict : dictList){
			if("".equals(action)){
				dict.setTreeurl("planTab/list.do?DICTIONARIES_ID="+dict.getDICTIONARIES_ID()+"&PARENT_ID="+dict.getPARENT_ID());
			}
			dict.setSubDict(this.listAllDictByPlanType(dict.getDICTIONARIES_ID(), action));
			dict.setTarget("treeFrame");
		}
		return dictList;
	}
